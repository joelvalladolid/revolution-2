try:
    ...
    ta = None  # solo usado en calcular_indicadores_tecnicos (backtesting), no en scanner
    _PANDAS_TA_OK = False
import pandas as pd
import numpy as np
...
def evaluar_protocolo_accion(data, tech, tnx_yield, price, soportes=None, profile='B', scan_mode='DIP'):
    ...
    # None si no hay desc (Yahoo no siempre devuelve, o devuelve '' vacío)
    if not desc:  # cubre None Y string vacío ''
        verdicts.append(("Negocio comprensible", None))
    else:
        verdicts.append(("Negocio comprensible", True))
    
    # Regla 2: Cash > Deuda
    verdicts.append(("Cash > Deuda (o Estratégica)", cash_gt_debt if data.get('total_cash') is not None or data.get('current_ratio') is not None else None))
    
    # Regla 3: Rentabilidad (None si no hay datos de márgenes)
    gross_m = data.get('gross_margins')
    profit_m = data.get('profit_margins')
    if gross_m is None and profit_m is None:
        verdicts.append(("Rentabilidad", None))
    else:
        verdicts.append(("Rentabilidad", bool((gross_m or 0) > 0 and (profit_m or 0) > 0)))
    ...
    # Regla 12: Anti-Dilución (Shares Growth <= 2%) — None si no hay dato
    shares_g = data.get('shares_growth')
    if shares_g is not None:
        dilucion_ok = (shares_g <= 0.02)
        verdicts.append(("No Dilución (Acciones ≤ +2%)", dilucion_ok))
    else:
        dilucion_ok = True  # beneficio de la duda si no hay dato
        verdicts.append(("No Dilución (Acciones ≤ +2%)", None))
    
    # Regla 13: Eficiencia Capital (ROIC o ROE > 10%) — None si no hay dato
    roic = data.get('roic')
    roe = data.get('roe')
    roa = data.get('roa')
    ...
    if val_eff is not None:
        ...
        eff_ok = True  # beneficio de la duda si no hay dato
        verdicts.append(("Eficiencia Capital (ROIC/ROE > 10%)", None))
    ...
    # Si total < min_stars, escalamos el umbral proporcionalmente
    # (evita que Yahoo devuelva pocos datos y bloquee todo)
    MIN_DATA_POINTS = 5  # m\u00ednimo de checks con datos para evaluar
    if total >= min_stars:
        pasa_estrellas = passed >= min_stars
    elif total >= MIN_DATA_POINTS:
        # Escalar: exigir el mismo RATIO que min_stars/total_esperado
        ratio_req = min_stars / 15.0  # 15 = total m\u00e1x de checks posibles
        pasa_estrellas = passed >= max(MIN_DATA_POINTS * ratio_req, 3)
    else:
        pasa_estrellas = False  # menos de 5 datos = no evaluar
