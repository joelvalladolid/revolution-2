...
def test_daily_allocator():
    ...
    # Cap por ticker: ≤15% en CALM
    for a in allocs:
        if a["ticker"] != "CASH":
            assert a["weight_pct"] <= 15.01, f"{a['ticker']} supera cap 15%: {a['weight_pct']}"

run_test("daily_allocator — Vol Parity, caps 15%, suma 100%, régimen CALM", test_daily_allocator)
