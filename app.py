...
def full_reset():
    ...
    # 3. Caché de funciones gestionado enteramente por Streamlit
...
if page == "📡 Radar S&P 500":
    ...
    def run_aapl_health_check(t_yield):
        try:
            stars, checks = get_fundamental_stars("AAPL", t_yield, 150.0)
            if any(c[0] == "Error de API (Yahoo bloqueó la conexión)" for c in checks):
                return False, "Yahoo API Bloqueada (Error 401/429)"
            return True, "OK"
        except Exception as e:
            return False, str(e)
    ...
    def scan_worker(sid, tickers, current_regime, t_yield, s_date, e_date, mode_list):
        ...
        ok, msg = run_aapl_health_check(t_yield)
        if not ok:
            state['status'] = 'error'
            state['error_msg'] = f"Health Check Falló (AAPL): {msg}. Scan cancelado."
            return
    ...
    def render_live_radar():
        ...
        if state['status'] == 'running':
            ...
            st.error(f"🚨 **ERROR CRÍTICO:** {state.get('error_msg', 'Falló la conexión')}")
            st.warning("Yahoo Finance está bloqueando las consultas desde la IP actual de Streamlit Cloud. Intenta nuevamente más tarde.")
