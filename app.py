...
def get_current_regime():
    try:
        from lab.macro_vix import get_vix_regime
        res = get_vix_regime()
        if res["status"] == "OK":
            return res["regime"], res["max_positions"], res["current_vix"]
        return "CALM", 10, 15.0
...
def analyze_ticker_for_today(ticker: str, regime: str, tnx_yield: float,
    ...
    try:
        ...
        # VIX_3_0: Sobrescribir signal_res para que dependa exclusivamente de MC
        signal_res = {
            "signal": True,
            "fundamental_ok": True, # Desactivamos bloqueo por estrellas
            "confidence": 0.0,
            "rationale": "Evaluando riesgo futuro",
            "indicators_active": [],
        ...
        if signal_res['signal']:
            try:
                ...
                horizon = 5 # Swing incondicional de 5 dias
                ...
                if p_gt_2 < 0.35 or p10 < -0.10:
                    ...
                    signal_res['rationale'] = f"Riesgo/Beneficio VIX_3_0 Falla: P(>2%)={p_gt_2*100:.1f}%, Riesgo P10={p10*100:.1f}%"
                else:
                    signal_res['confidence'] = mc.get('expected_value', 0) * 100
                    signal_res['rationale'] = f"VIX_3_0 PASS: EV={mc.get('expected_value', 0)*100:.2f}%"
            except Exception as e:
                signal_res['signal'] = False
                signal_res['rationale'] = f"Error Monte Carlo: {str(e)}"
        else:
            mc = None
        return {
            "ticker":      ticker,
...
regime, max_positions, current_vix = get_current_regime()
...
if page == "📡 Radar S&P 500":
    ...
    def process_ticker_all_modes(ticker, current_regime, t_yield, s_date, e_date, mode_list, preloaded_df, c_vix):
        ...
        for m in mode_list:
          ...
          # VIX_3_0 rankea por expected_value del Monte Carlo, igual que el backtest de +600%
          mc_data = res.get('mc')
          if mc_data and isinstance(mc_data, dict):
              proximity = mc_data.get('expected_value', 0.0) * 100
          else:
              proximity = 0.0
          ...
          if best_res is None or proximity > best_res['confidence']:
              best_res = res
        return best_res
    def scan_worker(sid, tickers, current_regime, t_yield, s_date, e_date, mode_list, c_vix, max_pos):
        ...
        if not state['cancel']:
          # Sort arrays by VIX_3_0 score (expected_value)
          state['results'] = sorted(
              state['results'], 
              key=lambda x: (x.get('mc', {}).get('expected_value', 0) if isinstance(x.get('mc'), dict) else 0), 
              reverse=True
          )
          # Limit by VIX_3_0 max positions
          if max_pos == 0:
              state['results'] = []
          else:
              state['results'] = state['results'][:max_pos]
    ...
    if scan_btn:
        ...
        threading.Thread(target=scan_worker, args=(session_id, sp500_list, regime, tnx_yield, start_date, end_date, modes_to_run, current_vix, max_positions), daemon=True).start()
    ...
    def render_live_radar():
        ...
        if state['status'] == 'running':
            ...
            if state['results']:
                temp_res = sorted(state['results'], key=lambda x: (x.get('mc', {}).get('expected_value', 0) if isinstance(x.get('mc'), dict) else 0), reverse=True)
                if max_pos > 0:
                    temp_res = temp_res[:max_pos]
                st.markdown(render_scanner_table(temp_res), unsafe_allow_html=True)
